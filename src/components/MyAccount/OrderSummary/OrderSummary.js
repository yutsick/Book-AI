...
<div className="md:hidden overflow-x-auto">
    ...
    <table className="border-collapse  w-full">
      <tbody>
        {headers.map((header, rowIndex) => (
          ...
          <td className="relative min-w-[185px] px-2 py-2 font-bold sticky left-0 bg-[#F4F4F4] z-10
  ...
  {header}
            ...
            {orders.map((order, colIndex) => (
              <td
                key={colIndex}
                className={`  px-2 py-1 border border-dark/70 ${
                  isSingleOrder ? "w-full" : "max-w-[160px]"
                }`}
              >
